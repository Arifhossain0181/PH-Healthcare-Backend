...
import { IUUpdatePatientProfilePayload,  } from  "./Patient.interface";
import { IUpdatePatientInfoPayload,   } from  "./Patient.interface";

export const updateMyPatientProfileMiddleware = (req : Request, res : Response, next : NextFunction) => { 
    if (req.body.data) {
        req.body = JSON.parse(req.body.data)
    }
    const payload: IUUpdatePatientProfilePayload = req.body;
  

    const files = req.files as { [fieldName: string]: Express.Multer.File[] | undefined };

    if (files?.profilePhoto?.[0]) {
        const patientInfo: IUpdatePatientInfoPayload = payload.patientInfo ?? (payload.patientInfo = {} as IUpdatePatientInfoPayload);
       patientInfo.profilePhoto = files.profilePhoto[0].path;
    }
