...
import { IUUpdatePatientProfilePayload, IUpdatePatientHealthDataPayload } from "./Patient.interface";
import { converdatetime } from "./Patient.utilits";

const updateMyProfile = async (user : IRequest , payload : IUUpdatePatientProfilePayload) => {
        ...
        if(payload.medicalReports && Array.isArray(payload.medicalReports) && payload.medicalReports.length > 0){
            for (const report of payload.medicalReports){
                if(report.shouldDelete && report.reportId){
                    ...
                    if (!report.doctorId || !report.appointmentId || !report.diagnosis || !report.treatment) {
                        throw new Error('Missing required medical report fields: doctorId, appointmentId, diagnosis, treatment');
                    }
